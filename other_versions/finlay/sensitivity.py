...
def cmd_run(args: argparse.Namespace) -> None:
    ...
    if not samples_path.exists() and not (out_dir / "samples.csv").exists():
        raise FileNotFoundError(f"Missing samples in {out_dir}; run sample first.")

    problem = load_problem(args.method, args.output_dir)
    samples = _load_samples(args.method, args.output_dir)
    n_rows = len(samples)
...
def _gather_partials(method: str, root: Path) -> pd.DataFrame:
    out_dir = method_dir(method, root)
    ...
    raise FileNotFoundError(f"No partials or {raw_path} in {out_dir}")


def _load_samples(method: str, root: Path) -> np.ndarray:
    """Load design matrix from .npy, or reconstruct from samples.csv."""
    mdir = method_dir(method, root)
    npy_path = mdir / "samples.npy"
    if npy_path.exists():
        return np.load(npy_path)
    csv_path = mdir / "samples.csv"
    if not csv_path.exists():
        raise FileNotFoundError(f"Missing {npy_path} and {csv_path}; run sample first.")
    problem = load_problem(method, root)
    df = pd.read_csv(csv_path)
    samples = df[problem["names"]].to_numpy(dtype=float)
    np.save(npy_path, samples)
    return samples


def _analysis_outputs(raw_df: pd.DataFrame) -> list[str]:
    """OUTPUTS present in a results table (Snellius runs may omit some)."""
    present = set(raw_df.columns)
    return [o for o in OUTPUTS if o in present]


def _complete_morris_row_indices(
    completed: set[int], n_samples: int, n_params: int
) -> list[int]:
    """Keep only rows belonging to fully completed Morris trajectories."""
    traj_size = n_params + 1
    n_traj = n_samples // traj_size
    complete_trajs = [
        t
        for t in range(n_traj)
        if all(r in completed for r in range(t * traj_size, (t + 1) * traj_size))
    ]
    return [r for t in complete_trajs for r in range(t * traj_size, (t + 1) * traj_size)]
...
def cmd_status(args: argparse.Namespace) -> None:
    ...
    for method in methods:
        ...
        csv_path = mdir / "samples.csv"
        if not samples_path.exists() and not csv_path.exists():
            print(f"{method}: no samples (run sample first)")
            continue
        n_samples = len(_load_samples(method, args.output_dir))
...
def plot_sobol_bars(
    ...
    outs = outputs or sorted(sobol_df["output"].unique())
    fig, axes = plt.subplots(len(outs), 1, figsize=(8, 2.2 * len(outs)), sharex=True)
    if len(outs) == 1:
        axes = [axes]
...
def plot_sobol_heatmap(
    ...
    pivot = pivot.reindex(index=outs, columns=params)
    ...
    ax.set_yticklabels([o.replace("_", " ") for o in outs])
...
def plot_morris_scatter(
    morris_df: pd.DataFrame, path: Path, outputs: list[str] | None = None
) -> None:
    outs = outputs or sorted(morris_df["output"].unique())
    fig, axes = plt.subplots(len(outs), 1, figsize=(6, 2.2 * len(outs)))
    if len(outs) == 1:
        axes = [axes]
    for ax, out in zip(axes, outs):
...
def analyze_sobol(raw_df: pd.DataFrame, problem: dict, out_dir: Path) -> pd.DataFrame:
    ...
    outs = _analysis_outputs(raw_df)
    Y = {out: raw_df[out].values for out in outs}
    rows = []
    for out in outs:
    ...
    plot_sobol_bars(sobol_df, params, out_dir / "sobol_first_order.png", index="S1", outputs=outs)
    plot_sobol_bars(sobol_df, params, out_dir / "sobol_total_order.png", index="ST", outputs=outs)
    plot_sobol_heatmap(sobol_df, params, out_dir / "sobol_heatmap.png", index="ST", outputs=outs)
    return sobol_df
...
def analyze_morris(
    ...
    completed = set(raw_df["row_index"].astype(int))
    keep = _complete_morris_row_indices(completed, n_samples, len(params))

    traj_size = len(params) + 1
    n_traj = n_samples // traj_size
    n_complete = len(keep) // traj_size
    if n_complete < n_traj:
        print(
            f"Morris: using {n_complete}/{n_traj} complete trajectories "
            f"({len(keep)}/{n_samples} rows)"
        )
    if n_complete < 2:
        raise RuntimeError(
            f"Too few complete Morris trajectories ({n_complete}); need at least 2."
        )

    raw_df = raw_df[raw_df["row_index"].isin(keep)].sort_values("row_index")
    X = samples[keep]
    outs = _analysis_outputs(raw_df)
    Y = {out: raw_df[out].values for out in outs}
    rows = []
    ...
    plot_morris_scatter(morris_df, out_dir / "morris_scatter.png", outputs=outs)
    return morris_df
